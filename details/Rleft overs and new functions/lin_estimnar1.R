...
logl_lin_new <- function(ca, N, TT, y, W) {
  wy <- cbind( 1, as.vector( W %*% y[, -TT] ), as.vector( y[, -TT] ) )
  lambdat <- as.vector( wy %*% ca)
  - sum( y[, -1] * log(lambdat) - lambdat) 
}

logl_lin_old <- function(ca, N, TT, y, W) {
  
  unon <- matrix(1, N, 1)
  g <- 0
  
  for ( ti in 2:TT ) {
    Xt <- cbind(unon, W %*% y[, ti - 1], y[, ti - 1])
    lambdat <- Xt %*% ca
...
scor_lin_new <- function(ca, N, TT, y, W) {
  wy <- cbind( 1, as.vector( W %*% y[, -TT] ), as.vector( y[, -TT] ) )
  lambdat <- as.vector( wy %*% ca )
...
scor_lin_old <- function(ca, N, TT, y, W) {
...
outer_lin_new <- function(ca, N, TT, y, W) {
...
outer_lin_old <- function(ca, N, TT, y, W) {
...
hess_lin_new <- function(ca, N, TT, y, W) {
  wy <- cbind( 1, as.vector( W %*% y[, -TT] ), as.vector( y[, -TT] ) )
  ct <- as.vector(y[, -1]) / as.vector( wy %*% ca )^2
  crossprod( wy * ct, wy )
}

hess_lin_old <- function(ca, N, TT, y, W) {
