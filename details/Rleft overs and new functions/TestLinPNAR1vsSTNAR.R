...
scor_stnar1_h0 <- function(ca, N, TT, y, W) {
...
scor_star1_h0_old <- function(c, N, TT, y, W){
...
outer_star1_h0 <- function(ca, N, TT, y, W) {
...
outer_star1_h0_old <- function(c, N, TT, y, W){
...
hess_star1_h0 <- function(ca, N, TT, y, W) {
  
  ca[5] <- 0
  Wt <- as.vector( W %*% y[, -TT] )
  wy <- cbind( 1, Wt, as.vector( y[, -TT] ), 0, exp( - ca[4] * Wt^2 ) * Wt )
  ct <- as.vector(y[, -1]) /  as.vector( wy[, 1:3] %*% ca[1:3] )^2
  crossprod( wy * ct, wy )

}

hess_star1_h0_old <- function(c, N, TT, y, W){
...
scor_star1_j <- function(ca, N, TT, y, W, msn) {
...
scor_star1_j_old <- function(c, N, TT, y, W, msn){
...
LM_gamma_stpnar <- function(gama, par, N, TT, y, W){
  
  ca <- c(par, gama, 0)
  S <- scor_star1_h0(ca, N, TT, y, W)
  H <- hess_star1_h0(ca, N, TT, y, W)
  B <- outer_star1_h0(ca, N, TT, y, W)
...
LM_gamma_stpnar_old <- function(gamma, par, N, TT, y, W){
  
  c <- c(par, gamma, 0)
  S <- scor_star1_h0_old(c, N, TT, y, W)
  H <- hess_star1_h0_old(c, N, TT, y, W)
  B <- outer_star1_h0_old(c, N, TT, y, W)
...
optim_gamma_stpnar <- function(par, N, TT, y, W){
 optimise(f = LM_gamma_stpnar, c(0.001, 10), par, N, TT, y, W)
}

optim_gamma_stpnar_old <- function(par, N, TT, y, W){
  
  #derivative-free optimization
  #golden section search algorithm looks for solution in the interval (0.001,10)
  
  s_gam <- optimise(f=LM_gamma_stpnar_old, c(0.001,10), par, N, TT, y, W)
...
score_test_star1_j <- function(ca, N, TT, y, W, J, len, gam_L, gam_U) {
  ...
  for ( j in 1:J ) {
    ...
    for ( i in 1:len ) {  
      ca[4]<- gam[i]
      Sj <- scor_star1_j(ca, N, TT, y, W, snj)
      LMjv[i] <- as.numeric( Sj[5] )^2 / Sigma[i]
    } ##  end for ( i in 1:1en )
...
score_test_star1_j_old <- function(c, N, TT, y, W, J, l, gam_L, gam_U){
  ...
  for(j in 1:J){
    ...
    for(i in 1:l){
      ...
      c[4]<- gam[i]
      Sj <- scor_star1_j_old(c, N, TT, y, W, snj)
      H <- hess_star1_h0_old(c, N, TT, y, W)
      B <- outer_star1_h0_old(c, N, TT, y, W)
