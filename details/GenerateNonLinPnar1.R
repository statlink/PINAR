...
#########################################################################################
#########################################################################################
##  poisson.MOD2.nonlin() --- function to generate counts from Poisson non-linear 
##  PNAR(1) model with intercept drift and parameters:
##      d = intercept vector
...
poisson.MOD2.nonlin <- function(d, B, W, gamma, Time, N, copula, rho, df)
{
  ...
  xstart  =t(t(-log(ustart))/lambda[,1])
...
#########################################################################################
#########################################################################################
##  poisson.MOD2.star() --- function to generate counts from Poisson non-linear 
...
poisson.MOD2.star <- function(d, B, W, gamma, a, Time, N, copula, rho, df)
{
  p2R <- rho^(1:(N-1))      ### creates column vectors of corr matrix R=(rho^|i-j|)_(i,j),
  R=toeplitz(c(1,p2R))      ### this is a symmetric Toeplitz matrix
  ...
  c[,1] <- a*exp(-gamma*(W%*%y[,1]*W%*%y[,1]))
  
  for (t in 2:Time){
    
    lambda[,t]=d+B%*%y[,t-1]+c[,t-1]*W%*%y[,t-1]
...
poisson.MOD2.tar <- function(d, B, W, gamma, a, Time, N, copula, rho, df)
{
  p2R <- rho^(1:(N-1))      ### creates column vectors of corr matrix R=(rho^|i-j|)_(i,j),
  R=toeplitz(c(1,p2R))      ### this is a symmetric Toeplitz matrix
  ...
  for (t in 2:Time){
    ...
    It <- 0+(W%*%y[,t]<=gamma)
    Xt <- cbind(unon, W%*%y[,t], y[,t], It, W%*%y[,t]*It, y[,t]*It, zeron)
    c[,t] <- Xt[,4:6]%*%a
