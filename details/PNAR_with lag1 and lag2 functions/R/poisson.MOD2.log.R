poisson.MOD2.log <- function(d, B, TT, N, copula = "gaussian", corrtype = "equicorrelation", rho, dof = 1) {
  ...
  x  <- Rfast::eachrow( log(u), - exp(nu[, 1]), oper = "/" )
  y[, 1] <- getN(x, tt = 1)
  x <- u <- NULL

  for ( ti in 2:TT ) {

    nu[, ti] <- d + B %*% log1p(y[, ti - 1])

    u <- rcopula(n, N, rho, copula, corrtype, dof, cholR)
    x  <- Rfast::eachrow( log(u), - exp(nu[, ti]), oper = "/" )
    y[, ti] <- getN(x, tt = 1)
    x <- u <- NULL
  }

  list(p2R = p2R, log_lambda = nu, y = y)
}
...
##      log_lambda = NxTT matrix of generated Poisson log(mean) for N time series over TT
##      p2R = Toeplitz correlation matrix employed in the copula
#############################################################################################

# poisson.MOD2.log_old <- function(d, B, TT, N, copula, rho, dof) {
#
...
#   if ( copula == "student" )       u <- rCopula(100, tCopula(param = p2R, dispstr = "toep", df = dof, dim = N) )
