...
################################################################################
################################################################################
##  LM_gama_tpnarpq() --- Function to optimize the score test statistic of
##  Threshold model (TNAR)  with p lags, under the null assumption of linearity,
##  with respect to unknown nuisance parameter (gama); 
##  it also includes q non time-varying covariates.
##  Input:
...
################################################################################
################################################################################
##  global_optimise_LM() --- Function to optimize the LM_gama_tpnarpq() 
...
##  Input:
##    f=LM_gama_tpnarpq function to be optimize. Always report it in the call.
...
global_optimise_LM <- function(f=LM_gama_tpnarpq, L, U, I, b, N, TT, y, W, p, d, Z, tol= 1e-14){
  ...
  for(i in 1:(I-1)){
    ...
    int <- c(x[i],x[i+1])
    opt <- optimise(f=LM_gama_tpnarpq, int, b, N, TT, y, W, p, d, Z, tol= tol)
  ...
  return(list(gama=gama, supLM=-1 * supLM, int=x))
...
################################################################################
################################################################################
##  LM_gama_tpnarpq_j() --- Function to optimize the perturbed version of 
...
LM_gama_tpnarpq_j <- function(gama, b, N, TT, y, W, p, d, Z, msn){
...
##  Inputs:
##     supLM = value of the score test at the optimum gama
##     b = estimated parameters from the linear model, in the following order:
##         (intercept, p network effects, p ar effects, covariates)
##     N = number of nodes on the network
##     TT = temporal sample size
...
##        gamaj = optimal values of gama parameter for score test boostrap replications
##        supLMj = values of perturbed test statistic at the optimum point gamaj
############################################################################################
...
score_test_tarpq_j<- function(supLM, b, N, TT, y, W, p, d, Z, J, L, U, tol = 1e-14){
  ...
  for(j in 1:J){
    ...
    opttj <- optimise(f=LM_gama_tpnarpq_j, c(L,U), b, N, TT, y, W, p, d, Z, msn=snj, tol=tol)
    gamaj[j] <- opttj$minimum
    supLMj[j] <- -1* opttj$objective
    
    #supLMj[j]<- -1 * LM_gama_tpnarpq_j(gama, b, N, TT, y, W, p, d, Z, msn=snj)
  ...
  return(list(pJ=pJ, cpJ=cpJ, supLMj=supLMj, gamaj=gamaj))
