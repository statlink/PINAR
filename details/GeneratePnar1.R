...
################################################################################
## Function for generating linear Poisson NAR model
################################################################################
...
adja <- function(N, K, alpha){
  ...
  gra <- igraph::sample_sbm(n=N, pref.matrix = pm, block.sizes=rep(N/K, K), directed = TRUE)
...
poisson.MOD2 <- function(d, B, Time, N, copula, rho, df) {
  ...
  if (copula == "gaussian") ustart <- rCopula(1000, normalCopula(param=p2R, dispstr = "toep", dim=N)) 
